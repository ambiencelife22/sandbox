...
const allowedChannels = ['Ambience', 'Glow', 'Glow+']
...
function BrickCelestial({ className, icon, userSessionToken, NebulaParam, currentChannel }) {
    ...
    const isChannelAllowed = NebulaParam === 'freebies' || allowedChannels.includes(currentChannel)

    let targetURL = `/${icon.CelestialParam}`
    ...
    if (!isChannelAllowed || icon.contentStatus === 'Not Launched') {
        targetURL = null
    }

    return targetURL ? <Link href={targetURL}>{cardContent}</Link> : cardContent
}
...
function getCardclassName(token, icon, isChannelAllowed) {
    ...
    if (icon.contentStatus === 'Not Launched') {
        conditionClasses += ' dashboard_brick brick !cursor-not-allowed'
    }
...
function getBackgroundImageStyle(img, isIcon) {
    let position = isIcon ? 'center' : 'center'
    let size = '55% 55%'

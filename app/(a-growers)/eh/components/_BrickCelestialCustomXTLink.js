...
function BrickCelestialCustomXTLink({ className, icon, userSessionToken, NebulaParam, currentChannel }) {
    ...
    const cardContent = (
        <motion.div key={icon.id} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.2 }}>
            <div className={`${cardclassName} ${icon.premium != 'NO' ? 'al_premium_icon_bg' : ''} telegram_icon al_premium_icon_bg`} style={backgroundImageStyle}>
    ...
    return icon.contentStatus !== 'Not Launched' ? (
        <a href={targetURL} target='_blank' rel='noopener noreferrer' onClick={openInNewTab}>
            {cardContent}
        </a>
    ) : (
        cardContent
...
export default BrickCelestialCustomXTLink
